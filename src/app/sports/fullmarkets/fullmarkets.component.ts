...
export class FullmarketsComponent implements OnInit {
                      ...
                      _.forEach(oddsData, (market, index:any) => {
                        if (market.MarketId == element2.MarketId) {
                          ...
                          if (market.marketName.indexOf('Winner ') > -1) {
                            market.Runners = market.Runners.filter(function (runner:any) {
          ...
          if (BackRunner) {
            _.forEach(runner.ExchangePrices.AvailableToBack, (value: any, index3: number) => {
              ...
              if (value.price != BackRunner[index3]?.price || value.size != BackRunner[index3]?.size) {
                const back = $('#' + runner.SelectionId + ' .back' + (index3 + 1));
                back.addClass('spark');
  ...
  getSportsBookMarket(sportsBookMarket: any[]) {
    ...
    _.forEach(sportsBookMarket, (market, index) => {
      ...
      if (market.sportsBookSelection) {
        market.sportsBookSelection = market.sportsBookSelection.sort((a: any, b: any) => {
          return parseInt(a.id) - parseInt(b.id);
        });
      }
  ...
  OpenBetSlip(event: any, sportId: any, tourid: any, matchBfId: any, bfId: any, SelectionId: any, matchName: any, marketName: any, isInplay: any, runnerName: any, odds: any, backlay: any, score: any, rate: any, fancyId: any, bookId: any, runnerId: any, bookType: any, sportBookType: any) {
    // console.log("click");
